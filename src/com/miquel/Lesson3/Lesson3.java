...
public class Lesson3 {
    ...
    static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
        ...
        if (parallel) {
            intConsumer = getParallelConsumer(wordList, distances);

        } else {
            intConsumer = getSequentialConsumer(wordList, distances);
        }
        IntStream.range(0, LIST_SIZE).forEach(intConsumer);
        return distances;
    }

    @NotNull
    private static IntConsumer getSequentialConsumer(List<String> wordList, int[][] distances) {
        return i -> wordList.
                forEach(s1 -> distances[i] = wordList.
                        stream().
                        mapToInt(s2 -> Levenshtein.lev(s1, s2)).toArray());
    }

    @NotNull
    private static IntConsumer getParallelConsumer(List<String> wordList, int[][] distances) {
        return i -> wordList.
                forEach(s1 -> distances[i] = wordList.
                        parallelStream().
                        mapToInt(s2 -> Levenshtein.lev(s1, s2)).toArray());
    ...
    static List<String> processWords(List<String> wordList, boolean parallel) {
        // YOUR CODE HERE
        List<String> wordsPrecessed = null;
        if (parallel) {
            wordsPrecessed = wordList.parallelStream()
                    .distinct()
            ...
            wordsPrecessed = wordList.stream()
                    .filter(s-> s.charAt(0) == 'a')
                    .distinct()
                    .map(String::toUpperCase)
                    .sorted(Comparator.comparingInt(String::length))
